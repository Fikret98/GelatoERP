...
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER -- Use SECURITY DEFINER to bypass RLS for aggregate analytics
...
BEGIN
    ...
    -- Using COALESCE to ensure we never return NULL to the frontend stats
...
-- IMPORTANT: Grant execute permission to both authenticated and anon roles
