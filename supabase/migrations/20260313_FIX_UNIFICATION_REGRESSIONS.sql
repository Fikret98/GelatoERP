-- FIX REGRESSIONS FROM UNIFICATION
-- 1. Recreate all_transactions_view (Essential for Reports page)
DROP VIEW IF EXISTS public.all_transactions_view;
...
-- 2. Update process_sale to remove shift_id dependency
CREATE OR REPLACE FUNCTION public.process_sale(
    p_total_amount numeric, 
    p_items jsonb, 
    p_seller_id integer,
    p_payment_method text DEFAULT 'cash',
    p_shift_id bigint DEFAULT NULL -- Kept as optional but ignored to avoid frontend breaking immediately
)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Record the sale (No shift_id column anymore)
...
GRANT EXECUTE ON FUNCTION public.process_sale(numeric, jsonb, integer, text, bigint) TO authenticated, anon;
